...
template <typename T>
struct is_std_string_like<std::basic_string<T>> : std::true_type {};

template <typename T>
struct is_std_string_like<std::basic_string_view<T>> : std::true_type {};
...
template <typename Context> struct arg_mapper {
  ...
  FMT_MAP_API auto map(char8_t val) -> char_type { return val; }
  ...
  template <typename T, FMT_ENABLE_IF(std::is_same<char, T>::value && std::is_same<char8_t, char_type>::value)>
  FMT_MAP_API auto map(T* val) -> const char_type* { return (char_type *)val; }

  template <typename T, FMT_ENABLE_IF(std::is_same<char, T>::value && std::is_same<char8_t, char_type>::value)>
  FMT_MAP_API auto map(const T* val) -> const char_type* { return (const char_type *)val; }

  template <typename T, FMT_ENABLE_IF(std::is_same<char8_t, T>::value && std::is_same<char, char_type>::value)>
  FMT_MAP_API auto map(T* val) -> const char_type* { return (char_type *)val; }
