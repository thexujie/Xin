...
public:
	...
	{
		Xin::Atomics::IncFetch(DllRefCount);
...
IFACEMETHODIMP CNavigateToOriginalPathCommandVerb::GetState(IShellItemArray * ShellItemArray, BOOL fOkToBeSlow, EXPCMDSTATE * pCmdState)
{
	...
	TCoMemPtr<wchar_t> CurrShellItemFilePath;
	CurrShellItem->GetDisplayName(SIGDN_FILESYSPATH, CurrShellItemFilePath.GetPP());

	// The shell treats ECS_HIDDEN the same as ECS_DISABLED for context menu items added via IExplorerCommand
	//FStringView ModuleFilePath = Win32::GetModuleFilePath(Win32::GetAddressModule(IsShellItemLinked));
	//FStringView ModuleFileDirectory = FPath::GetDirectory(ModuleFilePath);

	//FString ConfigFilePath = FPath::Combine(ModuleFileDirectory, StrView("Config.ini"));
	//if (FFile::Exists(ConfigFilePath))
	//{
	//	FIniDocument IniDocument(ConfigFilePath);
	//	FStringView IncludeRegexString = IniDocument[StrView("General")][StrView("Include")];
	//	if (IncludeRegexString.Size)
	//	{
	//		FRegex IncludeRegex(IncludeRegexString);
	//		if (!IncludeRegex.Match(ModuleFilePath))
	//		{
