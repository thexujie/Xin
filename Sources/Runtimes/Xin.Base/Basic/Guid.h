...
namespace Xin { namespace Private_Guid
	{
#pragma pack(push, 1)
...
#pragma pack(pop)
		...
		inline uint8 GuidCharToUInt4(char ch)
		{
			if ('A' <= ch && ch <= 'F')
				return 10 + ch - 'A';
			else if ('a' <= ch && ch <= 'f')
		...
		inline uint8 GuidToUInt8(const char (& text)[2])
		{
			union
			{
				uint8 value;
		...
		inline uint16 GuidToUInt16(const char (& text)[2][2])
		{
			union
			{
				uint16 value;
		...
		inline uint32 GuidToUInt32(const char (& text)[2][2][2])
		{
			union
			{
				uint32 value;
		...
		inline char FromUInt4(uint8 _value, bool upper = false)
		{
		...
		inline void FromUInt8(uint8 _value, char (& text)[2], bool upper = false)
		{
			union
			{
				uint8 value;
		...
		inline void FromUInt16(uint16 _value, char (& text)[2][2], bool upper = false)
		{
			union
			{
				uint16 value;
		...
		inline void FromUInt32(uint32 _value, char (& text)[2][2][2], bool upper = false)
		{
			union
			{
				uint32 value;
	...
	struct BASE_API FGuid
	{
		...
		FGuid(uint32 Data1In, uint16 Data2In, uint16 Data3In, const uint8 (& Data4In)[8])
		{
			Data1 = Data1In;
			Data2 = Data2In;
			Data3 = Data3In;
			MemoryCopy(Data4, Data4In, 8);
		}

		// {0x34256d4b, 0xe67f, 0x4e05, 0x93, 0x60, 0xc, 0x86, 0x3a, 0x1c, 0x42, 0xc1}
		FGuid(uint32 Data1In, uint16 Data2In, uint16 Data3In,
			uint8 Data40In, uint8 Data41In, uint8 Data42In, uint8 Data43In,
			uint8 Data44In, uint8 Data45In, uint8 Data46In, uint8 Data47In)
		{
			Data1 = Data1In;
			Data2 = Data2In;
			Data3 = Data3In;
