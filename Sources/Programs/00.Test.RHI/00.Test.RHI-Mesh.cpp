...
FBX::FFbxModel LoadMeshWithCache(FStringV FbxFilePath)
{
	...
	FStringV CacheDirectory = u8"../../Documents/FbxMeshCache"V;
...
public:
	...
	FStaticMeshVS() : IShaderSource(EShaderStage::Vertex, FPath::Combine(GetProjectAssetsPath(), u8"Shaders/Image.xsf"V), u8"MainVS"V) {}
};
...
public:
	...
	FStaticMeshPS() : IShaderSource(EShaderStage::Pixel, FPath::Combine(GetProjectAssetsPath(), u8"Shaders/Image.xsf"V), u8"MainPS"V) {}
};

int TestRHI_Mesh(FEngine & Engine)
{
	TestRHI_Initialize();
	...
	//FBX::FFbxModel FbxModel = LoadMeshWithCache(u8"../../Data/SKM_Manny.fbx"V);

	FBX::FFbxModel FbxModel = LoadMeshWithCache(u8"../../Data/QShanTu.fbx"V);
	...
	//FBX::FFbxModel FbxModel = LoadMeshWithCache(u8R"(H:\Files\Models\白色裙子女生\Girl.fbx)"V);
